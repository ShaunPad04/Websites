  ...
  return (
    <>
      <button
        ...
        className={`group relative z-[60] inline-flex shrink-0 items-center gap-2.5 rounded-full px-5 py-2.5 text-sm tracking-[0.08em] transition-colors ${
          open || scrolled
        ...
        {open ? (
          <>
            ...
            <motion.div
              ...
              aria-modal="true"
              aria-label="Menu"
              className="pointer-events-auto absolute right-4 top-0 w-[min(26rem,calc(100vw-2rem))] sm:right-6 origin-top-right overflow-hidden rounded-[22px] bg-red text-bone shadow-[0_30px_80px_rgba(12,10,9,.5)]"
              style={{ maxHeight: "calc(100svh - 1.5rem)" }}
              {...panelMotion}
            >
              <div className="grain relative flex max-h-[calc(100svh-1.5rem)] flex-col overflow-y-auto p-6 pt-20 sm:p-8 sm:pt-24">
                  ...
                  {MENU.map((item, i) => (
                    ...
                    >
                      <a
                        href={item.href}
                        onClick={close}
                        className="group flex min-h-[44px] items-baseline gap-4 py-3.5 transition-[padding] duration-200 ease-out hover:pl-2 focus-visible:pl-2 sm:py-4"
                      >
                        <span className="label w-6 shrink-0 text-bone/45 transition-colors duration-200 group-hover:text-gold-lift group-focus-visible:text-gold-lift">
                          {item.n}
                        </span>
                        <span className="display text-[1.6rem] leading-none tracking-[-0.01em] sm:text-[2rem]">
                          {item.label}
                        </span>
                ...
                >
                  <a
                    ...
                    className="group flex w-full items-center justify-between gap-3 rounded-full bg-bone px-6 py-4 text-onyx transition-colors hover:bg-white"
                  >
                    <span className="label">Find us</span>
                    <span aria-hidden="true"
                      className="grid h-8 w-8 place-items-center rounded-full bg-onyx text-bone transition-transform duration-300 ease-out group-hover:rotate-45">
                  ...
                  <address className="mt-6 not-italic text-sm leading-relaxed text-bone/70">
                    {addressLines.map((l) => (
                  ...
                  <p className="mt-2 text-sm text-bone/50">Tuesday to Sunday, 10 till 4</p>
