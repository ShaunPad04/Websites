...
Read `design-system.md` from the project root if it exists and follow it as the
authoritative token source. If it does not exist, treat that as NO_DESIGN_SYSTEM_FILE
and fall back to the defaults below.

<!-- NOTE: this was a dynamic context injection (bang-prefixed shell) upstream. Claude Code
     never executes those in a skill synced from a claude.ai account, so it is
     written as an explicit read instruction, which works in every context. -->

### If design-system.md exists and contains actual design system content:
